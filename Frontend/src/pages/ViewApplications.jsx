...
const ViewApplications = () => {
  ...
  const fetchCompanyJobApplicantions=async()=>{
    try {
      const {data}=await axios.get(`${backendUrl}/api/v2/company/applicants`,{withCredentials:true})
      if(data.success){
  ...
  const changeJobApplicationStatus=async(id,status)=>{
    try {
      const {data}=await axios.post(`${backendUrl}/api/v2/company/change-status`,{id,status},{withCredentials:true})
      if(data.success){
